...
const diffColorMap: Record<DiffLevel, string> = {
  '极难': 'bg-red-500', '难': 'bg-orange-500', '中高': 'bg-yellow-500',
  '中等': 'bg-lime-500', '中低': 'bg-green-500', '较易': 'bg-teal-500',
};
const diffBgMap: Record<DiffLevel, string> = {
  '极难': 'bg-red-50', '难': 'bg-orange-50', '中高': 'bg-yellow-50',
  '中等': 'bg-lime-50', '中低': 'bg-green-50', '较易': 'bg-teal-50',
};
const diffTextMap: Record<DiffLevel, string> = {
  '极难': 'text-red-700', '难': 'text-orange-700', '中高': 'text-yellow-700',
  '中等': 'text-lime-700', '中低': 'text-green-700', '较易': 'text-teal-700',
};
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="animate-fade-up">
        ...
        <p className="text-sm text-muted-foreground">全国 28 省份竞争热度 · 地市难度分布 · 岗位类型可视化</p>
      </div>
        ...
        {views.map(v => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all flex-1 ${
              activeView === v.key ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
        ...
        <div className="space-y-5 animate-fade-up">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            <div className="flex gap-1.5 flex-wrap">
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full transition-colors ${
                  diffFilter === 'all' ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-muted/80'
                }`}
              >全部</button>
              {DIFFICULTY_SCALE.map(d => (
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-full transition-colors ${
                    diffFilter === d.level ? `${diffBgMap[d.level]} ${diffTextMap[d.level]} font-medium border ${diffColorMap[d.level]}/20` : 'bg-muted text-muted-foreground hover:bg-muted/80'
                  }`}
                >{d.level}</button>
              ))}
            </div>
            <div className="relative ml-auto">
              ...
              <input
                ...
                placeholder="搜索省份"
                className="pl-9 pr-3 py-1.5 text-xs rounded-lg border bg-background focus:outline-none focus:ring-1 focus:ring-primary/30 w-32"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
            {filteredProvinces.map((p, i) => (
              ...
              >
                ...
                <div className="text-[10px] text-muted-foreground mt-1">{p.avgRatio}:1</div>
              </button>
            ))}
          </div>
          ...
          {selectedData && (
            <Card className="p-5 animate-fade-up">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">平均竞争比 {selectedData.avgRatio}:1</span>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="text-center p-3 rounded-xl bg-muted/30">
                  <div className="text-2xl font-bold font-serif text-primary">{selectedData.posts.toLocaleString()}</div>
                  <div className="text-[10px] text-muted-foreground mt-1">招录岗位</div>
                </div>
                <div className="text-center p-3 rounded-xl bg-muted/30">
                  <div className="text-2xl font-bold font-serif">{selectedData.avgRatio}:1</div>
                  <div className="text-[10px] text-muted-foreground mt-1">平均竞争比</div>
                </div>
                <div className="text-center p-3 rounded-xl bg-muted/30">
                  <div className="text-sm font-bold font-serif">{selectedData.salary}</div>
                  <div className="text-[10px] text-muted-foreground mt-1">薪资范围</div>
                </div>
              </div>
              <div>
                <div className="text-xs font-medium text-muted-foreground mb-2">热门城市</div>
                <div className="flex gap-2 flex-wrap">
                  {selectedData.hotCities.map(c => (
                    <span key={c} className="text-xs px-3 py-1 rounded-full bg-primary/8 text-primary">
                      {c}
                    </span>
                  ))}
                </div>
              ...
              {/* 竞争比柱状图 */}
              <div className="mt-4">
                <div className="text-xs font-medium text-muted-foreground mb-2">竞争比可视化</div>
              ...
              {[...PROVINCE_DATA].sort((a, b) => b.avgRatio - a.avgRatio).slice(0, 10).map((p, i) => (
                <div key={p.id} className="flex items-center gap-3">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    i < 3 ? 'bg-red-100 text-red-700' : 'bg-muted text-muted-foreground'
                  }`}>{i + 1}</span>
                  ...
                  <span className="text-xs text-muted-foreground w-20 text-right">{p.posts} 岗</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="space-y-5 animate-fade-up">
          <Card className="p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary ml-auto">示例省份</span>
            </div>
            <p className="text-xs text-muted-foreground">点击各省概览页的省份卡片，可查看该省地市详情（当前以山东省为示例）</p>
          </Card>

          {/* 地市热力网格 */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {CITY_DETAIL_SD.map((c, i) => (
              <Card key={c.city} className={`p-4 card-hover border-l-4 animate-scale-in`} style={{ borderLeftColor: diffColorMap[c.difficulty].replace('bg-', '#'), animationDelay: `${i * 40}ms` }}>
              ...
              {CITY_DETAIL_SD.map((c, i) => {
                ...
                return (
                  <div key={c.city} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-[8px] text-muted-foreground font-medium">{c.ratio}</span>
                    <div
                    ...
                    />
                    <span className="text-[8px] text-muted-foreground truncate w-full text-center">{c.city}</span>
                  </div>
                );
              })}
            </div>
                ...
                <div key={p.type} className="animate-slide-right" style={{ animationDelay: `${i * 60}ms` }}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${diffBgMap[p.difficulty as DiffLevel] || 'bg-muted'} ${diffTextMap[p.difficulty as DiffLevel] || 'text-muted-foreground'}`}>
                        {p.difficulty}
                      </span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-semibold">{p.count.toLocaleString()}</span>
                      <span className="text-[10px] text-muted-foreground ml-1">({p.ratio}%)</span>
                    </div>
                  </div>
                  <div className="h-6 rounded-lg bg-muted/50 overflow-hidden relative">
                    <div
            ...
            <div className="flex items-center gap-6 justify-center">
              <div className="relative w-48 h-48">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[10px] text-muted-foreground">总岗位数</span>
                </div>
              </div>
              <div className="space-y-1.5">
                {POST_DISTRIBUTION.slice(0, 5).map(p => (
                  <div key={p.type} className="flex items-center gap-2 text-xs">
                    ...
                    <span>{p.type}</span>
                    <span className="text-muted-foreground">{p.ratio}%</span>
                  </div>
                ))}
          ...
          <Card className="p-5">
            ...
            <div className="space-y-3">
              {[...GUOKAO_SYSTEMS].sort((a, b) => b.ratio - a.ratio).map((s, i) => (
                <div key={s.name} className="p-4 rounded-xl border border-border/50 card-hover animate-slide-right" style={{ animationDelay: `${i * 50}ms` }}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        i < 3 ? 'bg-red-100 text-red-700' : 'bg-muted text-muted-foreground'
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-center w-20 shrink-0">
                      ...
                      <div className="text-[9px] text-muted-foreground">岗位数</div>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {GUOKAO_SYSTEMS.map((s, i) => {
                ...
                return (
                  <div key={s.name} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-[8px] text-muted-foreground">{(s.posts / 1000).toFixed(1)}k</span>
                    <div
                      className={`w-full rounded-t-md ${diffColorMap[s.difficulty]} animate-bar-rise`}
                      style={{ height: `${height}%`, animationDelay: `${i * 60}ms` }}
                    />
                    <span className="text-[7px] text-muted-foreground truncate w-full text-center leading-tight">{s.name.slice(0, 2)}</span>
                  </div>
                );
              })}
            </div>
