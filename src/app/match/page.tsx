    ...
    <div className="hero-reveal space-y-8 max-w-6xl">
      ...
      {animating && (
        <div className="flex flex-col items-center py-12 animate-fade-in">
          <div className="w-16 h-16 rounded-2xl bg-[#b4ff39]/10 flex items-center justify-center mb-4 animate-float">
            <Sparkles className="w-8 h-8 text-[#b4ff39]" />
          </div>
          <p className="text-sm text-zinc-500">AI 正在分析你的专业优势与岗位匹配度...</p>
          <div className="flex gap-1 mt-4">
            {[0,1,2].map(i => (
              <div key={i} className="w-2 h-2 rounded-full bg-[#b4ff39] animate-bounce" style={{animationDelay: `${i * 0.15}s`}} />
            ))}
          </div>
        </div>
      )}
      ...
      {matched && !animating && (
        <div className="hero-reveal space-y-6 animate-fade-up">
          ...
          <div className="topo-card p-5 border-[#b4ff39]/20 bg-[#b4ff39]/5">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-[#b4ff39]/10 flex items-center justify-center">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {[
                ...
                <div key={tier.label} className="topo-card p-4 animate-scale-in" style={{animationDelay:`${(i+1)*100}ms`}}>
                  <div className={`w-8 h-8 rounded-lg ${tier.bg} flex items-center justify-center mb-3 ${tier.accent}`}>
                    <tier.icon className="w-4 h-4" />
                  </div>
                  <h4 className="font-semibold text-sm mb-2">{tier.label}</h4>
                  <div className="flex flex-wrap gap-1">
                    {tier.cities.map(c => (
                      <span key={c} className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.04] text-zinc-400">{c}</span>
                    ))}
                  </div>
                </div>
              ))}
